...
public class LearnDropsCommand {

    public final String[] DROPS = { "seafood_raw", "wolfmeat_raw" };

    @SubscribeEvent
    public void onRegisterCommands(RegisterCommandsEvent event) {
        event.getDispatcher().register(
                Commands.literal("cle")
                        .then(Commands.literal("learn_drops")
                        ...
                        .executes(context -> {
                                    if (context.getSource().isPlayer()) {
                                        ...
                                        for(String s : DROPS){
                                            if(cultureRep.hasLearnedHuntingDrop(sp.getUUID(), s)){
                                                context.getSource().sendSystemMessage(Component.literal("I already know hunting drop " + "\"" + s + "\""));
                                            }
                                            else{
                                                cultureRep.learnHuntingDrop(sp.getUUID(), s);
